...
const SuyHaoBSC = () => {
  ...
  // Hàm kiểm tra xem ngày đã tồn tại trong Firebase hay chưa
  async function checkNgayExists(ngay) {
    try {
      const snapshot = await TuyenKTService.getAll_firebase(); // Lấy toàn bộ dữ liệu
      if (snapshot && snapshot.data) {
        const data = snapshot.data;
        return Object.values(data).some((item) => item.ngay === ngay); // Kiểm tra nếu ngày đã tồn tại
      }
      return false;
    } catch (error) {
      console.error("Lỗi khi kiểm tra ngày trong Firebase:", error);
      return false;
    }
  }

  // Hàm thêm dữ liệu với kiểm tra ngày trước khi chèn
  async function addDataIfNgayNotExists(newitem_bq8362) {
    console.log(newitem_bq8362);
    newitem_bq8362.id = generateRandomId();

    // Kiểm tra xem ngày đã tồn tại chưa
    const exists = await checkNgayExists(newitem_bq8362.ngay);

    if (!exists) {
      await TuyenKTService.add_data_8382_firebase({
        id: newitem_bq8362.id,
        data: newitem_bq8362,
      })
        .then((res) => {
          console.log("Dữ liệu đã lưu:", res.data);
        })
        .catch((err) => {
      ...
      console.log(
        `Dữ liệu với ngày ${newitem_bq8362.ngay} đã tồn tại, không chèn mới.`
      );
    }
  }

  const deleteAllData = () => {
    const dataRef = db.ref("baocaotuan"); // Thay 'yourDataNode' bằng tên node bạn muốn xóa
    dataRef
      .remove()
      .then(() => {
  ...
  const tuyenktbuyid = (ids) => {
    ...
    return list.filter((e) => e.id == ids)[0]
      ...
      : list.filter((e) => e.id == ids.split(":")[0])[0]?list.filter((e) => e.id == ids.split(":")[0])[0]:list.filter((e) => e.id == ids.split("/")[0])[0];
  };
      ...
      if (flag) {
        ...
        listTuyenKT_TT
          ...
          .map((item, index) => {
            ...
            // console.log(kemdptong, kemdptong, slcongkemdp);
            kemdptonghgi = Number(kemdptonghgi) + Number(slcongkemdp);
            congkemsochitieu =
              oltrx + onurx + "/" + (Number(item.sl) * (0.04 / 100)).toFixed(0);
            tylecongkem = ((kem / Number(item.sl)) * 100).toFixed(3);
            tylecongkem2dbm = ((slcongkemdp / Number(item.sl)) * 100).toFixed(
              3
            );
            ...
            if ((index % 5 == 0 && index > 0) || index == 20) {
              let th = {
                ...
                kemtn: (
                  ((Number(kemtong) - Number(kem)) /
                    Number(tongsl - tonghientai)) *
                  100
                ).toFixed(3),
                kem2dbm: (
                  ((Number(kemdptong) - Number(slcongkemdp)) /
                    Number(tongsl - tonghientai)) *
                  100
                ).toFixed(3),
                // tichluy: "0.00%",
        ...
        let th = {
          ...
          kemtn: ((kemtong / Number(tongsl)) * 100).toFixed(3),
          kem2dbm: ((kemdptong / Number(tongsl)) * 100).toFixed(3),
          // tichluy: "0.00%",
          nhanvien: listnv[i],
        };
        console.log(th);
        ...
        th = {
          ...
          kemtn: ((kemtonghgi / Number(tongslhgi)) * 100).toFixed(3),
          kem2dbm: ((kemdptonghgi / Number(tongslhgi)) * 100).toFixed(3),
          // tichluy: "0.00%",
          nhanvien: "",
        };
        ...
        addDataIfNgayNotExists(newitem_bq8362);
        // TuyenKTService.add_data_8382_firebase({
        //   id: newitem_bq8362.id,
        //   data: newitem_bq8362,
        // }).then((res) => {
        //   console.log(res.data);
  ...
  useEffect(() => {
    ...
    // TuyenKTService.add_data_8382_firebase(data).then(res=>{
    //   console.log(res.data)
    // })
    // deleteAllData();
    TuyenKTService.getAll_firebase().then((res) => {
  ...
  const deleteData = async () => {
    ...
    if (result.isConfirmed) {
      TuyenKTService.getAll_firebase().then(
        res=>{
          console.log(res.data)
          const data=Object.values(res.data).filter(e=>e.ngay==date)
          if (data.length>0)
          
          TuyenKTService.delete(data[0]).then(
            res=>{
              console.log(res.data)
            }
          )
          ...
          <table className="table mt-3 table-bordered   table-hover ">
            <thead className="thead-dark">
              <tr>
                ...
                <th scope="col">Tỷ lệ cổng kém DP 2dBm trong ngày</th>
